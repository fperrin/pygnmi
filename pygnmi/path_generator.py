...
def gnmi_path_generator(path_in_question: str, path_origin: str = None):
    ...
    - "yang-module:container/container[key=value]"; the origin set to yang-module

    - "/yang-module:container/container[key=value]"; identical to the previous
    ...
    if path_origin is not None:
        # if path origin given explicitely, use that
        gnmi_path.origin = path_origin

    elif path_elements and re.match('.+?:.*?', path_elements[0]):
        # otherwise if first path element contains a colon, use that to
        # set origin
